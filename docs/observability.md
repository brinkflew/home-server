...
| `home_server_network_info`, `home_server_container_attached`, `home_server_container_published_port` | **The podman socket is denied to `container_t`**, so nothing in a container can run `podman ps` or `podman network ls`. Added 2026-09-08 at **zero new subprocess cost** - the collector already made both calls and kept only the subnet map. `isolate` is a label rather than a gauge, the way `container_info` carries `pod`/`unit`/`image`, and reads EMPTY rather than `false` when the option is absent. The published port is the VALUE and the host side is the label: a publish is identified by where it is bound. |
