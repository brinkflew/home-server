  ...
  return {
    ...
    rounds: [
      ...
      // is the third and last thing a person is ever waiting on here.
      {
        ...
        started_at: iso(40 * 3600),
        ended_at: iso(36 * 3600),
        attempts: 1,
        max_attempts: 3,
        flow_job_id: "job-eee",
        head: "77aa31d",
      ...
      // round this fleet published before the feature shipped looks like this.
      {
        ...
        started_at: iso(200 * 3600),
        ended_at: iso(196 * 3600),
        attempts: 1,
        max_attempts: 3,
        flow_job_id: "job-jjj",
        head: "3c8c616",
      ...
      // Not the same outcome as the row below, and only this join says so.
      {
        ...
        started_at: iso(60 * 3600),
        ended_at: iso(55 * 3600),
        attempts: 1,
        max_attempts: 3,
        flow_job_id: "job-ggg",
        head: "1d2e3f4",
      ...
      // `closed_why` is shown to a reader and parsed by nothing.
      {
        ...
        held_at: null,
        held_why: null,
        error: "the gate failed on `e2e-test`",
        cost_usd: 17.94,
        tokens_in: 538200,
        tokens_out: 25116,
      },
      ...
      // untestable by eye and a regression in it invisible.
      {
        ...
        started_at: iso(120 * 3600),
        ended_at: iso(115 * 3600),
        attempts: 1,
        max_attempts: 3,
        flow_job_id: "job-iii",
        head: "5b6c7d8",
      ...
      // out of a paragraph is the parse this codebase refuses - so the chip is
      // disabled and the attempt line is hidden rather than guessed at "1 of 2".
      // Every round this fleet ran before 2026-08-28 is in this position.
      {
        worktree_id: "wt-hist01",
        project: "upskald",
        odoo_task: null,
        ref: null,
        phase: "verify",
        opened_at: iso(190 * 3600),
        started_at: iso(190 * 3600),
        ended_at: iso(188 * 3600),
        attempts: null,
        max_attempts: 3,
        flow_job_id: null,
        head: null,
        ...
        waiting_on: null,
        link: null,
        summary: "`api/src/core/api/pagination.py` has no unit tests",
        kind: null,
        closed_at: iso(188 * 3600),
        closed_why: null,
        done: ["plan", "dev"],
        phases: PHASES,
        odoo_url: null,
      ...
      // visible instead of collapsing into "attempt 2 of 2" on one row.
      {
        ...
        waiting_on: null,
        link: null,
        summary: "Build the requirements intake form",
        kind: null,
        closed_at: iso(3 * 3600),
        // THE SAME SENTENCE TWICE, and the row must print it once. conduct
        // builds closed_why as "the flow failed: <the refusal>", so on a
        // refused round it repeats the run's own reason with a prefix.
        closed_why:
        ...
        held_at: null,
        held_why: null,
        error: "the gate failed in the pristine tree (exit 2, e2e-test)",
        cost_usd: 17.57,
        tokens_in: 520000,
        tokens_out: 21000,
      },
    ...
    control: {
      ...
      // it is on is not in git.
      intake: [
        { subject: "upskald", value: "on", at: iso(3 * 3600),
          note: "catching up on the backlog" },
      ],
      holds: [
...
export function fleetUnreadable(): FleetDocument {
  ...
  return {
    ...
    runs: [],
    intake: [],
    // A LOCKED DATABASE CANNOT SAY WHAT ANYBODY ASKED FOR EITHER, so the
